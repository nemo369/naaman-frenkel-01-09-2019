...
async function getCityFiveDaysForcast(cityid) {
  const req = `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${cityid}?apikey=${apiKey}&language=en-us&details=false&metric=true%20HTTP/1.1`;
  return axios.get(req);
}
async function getCurrentWeather(cityid) {
  const req = `https://dataservice.accuweather.com/currentconditions/v1/${cityid}?apikey=${apiKey}&language=en-us&details=false&metric=true%20HTTP/1.1`;  
  return axios.get(req);
}
async function getAutoSearch(value) {
  const req =
     `https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=${apiKey}&q=${value}&language=en-us%20HTTP/1.1`;
  return axios.get(req);
}
